...
class DecisionService
{
    ...
    protected function getBucketingId($userId, $userAttributes)
    {
        ...
        if (isset($userAttributes[$bucketingIdKey]) && !is_null($userAttributes[$bucketingIdKey])){
            if (is_string($userAttributes[$bucketingIdKey])){
                return $userAttributes[$bucketingIdKey];
            }
        }
        $this->_logger->log(Logger::WARNING, 'Bucketing ID attribute is not a string. Defaulted to user_id.');
        return $userId;
    }
